...
<html>
<head>
<link href="http://fonts.googleapis.com/css?family=Open+Sans:300italic,400italic,600italic,700italic,800italic,300,400,600,700,800&amp;subset=latin,latin-ext,cyrillic,cyrillic-ext,greek,greek-ext,vietnamese" rel="stylesheet" type="text/css">
...
<script type="text/javascript">
		...
		var UBGradio = new buzz.sound("http://streaming.radionomy.com/UBGRadio?lang=en-GB");
		var streaming = false;
		...
		function player()
		{
			...
			{
				$('#volume').val(10);
				UBGradio.setVolume(0);
				UBGradio.fadeTo(10,10000);
				streaming = true;
				updateSong();
				setInterval("updateSong()",10000);
				updateArt();
				setInterval("updateArt()",10000);
			}
			else
			{
				$('#volume').val(10);
				UBGradio.setVolume(10);
				UBGradio.play();
				streaming = true;
				updateSong();
				setInterval("updateSong()",10000);
		...
		function toggle()
		{
		  ...
		  if(streaming)
		  {
			...
			$('.toggle').removeClass('fa-stop-circle');
			streaming = false; 
		  ...
		  else
		  {
			...
			UBGradio.unmute();
			$('.toggle').addClass('fa-stop-circle');
		...
		function setPlayerVolume()
		{
		  ...
		  UBGradio.setVolume($('#volume').val());
		}

		function updateSong()
			{			
				$.post("http:///ubgradio.chisdealhd.xyz/api/song/index.php",
				function(data) {
					$('.songname').text(data); 
				}); 
			}
...
<div id="player">
<div class="song">
<img width="230px" title="" alt="" src="https://cdn.discordapp.com/attachments/318740191131205634/372069875906117633/logo.png" />
<br />
<div class="songname">Currently Playing</div>
<iframe src="http://ubgradio.chisdealhd.xyz/api/artwork/index.php" frameborder="0" scrolling="no" width="100px" height="100px"></iframe>
</div>
<div class="player">
<center>
<a id="toggle" href="#" onclick="toggle()"><i class="toggle fa fa-stop-circle"></i></a><input id="volume" name="volume" min="0" max="100" step="1" type="range" onchange="setPlayerVolume()" value="50" />
</center>
</div>
</div>
</body>
